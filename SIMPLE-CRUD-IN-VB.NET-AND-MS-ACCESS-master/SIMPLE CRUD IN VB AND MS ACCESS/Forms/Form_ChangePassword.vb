...
Public Class Form_ChangePassword ' Created by: Joshua C. Magoliman
    ...
    Public Sub Validation()
        ...
        Else
            Try
                con.Open()
                cmd = New OleDbCommand("SELECT * FROM [tbl_Users] WHERE StrComp([username],@username, 0) = 0  AND StrComp([Password],@password , 0) = 0 ", con)
                cmd.CommandType = CommandType.Text
                cmd.Parameters.Add(New OleDbParameter("@username", usernameGetter))
                cmd.Parameters.Add(New OleDbParameter("@password", txtCurrentPassword.Text))
    ...
    Public Sub Reading()
        ...
        Else
            con.Open()
            cmd = New OleDbCommand("SELECT * FROM [tbl_Users] WHERE [username]=@username AND StrComp([Password],@password , 0) = 0 ", con)
            cmd.CommandType = CommandType.Text
            cmd.Parameters.Add(New OleDbParameter("@username", usernameGetter))
            cmd.Parameters.Add(New OleDbParameter("@password", txtConfirmNewPassword.Text))
            dr = cmd.ExecuteReader()
            Dim count As Integer
            count = 0
            While dr.Read
    ...
    Public Sub Updating()
        Try
            con.Open()
            cmd = New OleDbCommand("UPDATE [tbl_Users] set [password]=@password WHERE [username]=@username", con)
            cmd.CommandType = CommandType.Text
            cmd.Parameters.Add(New OleDbParameter("@password", txtNewPassword.Text))
            cmd.Parameters.Add(New OleDbParameter("@username", usernameGetter))
            cmd.ExecuteNonQuery()
            cmd.Dispose()
